...
from forms import LoginForm, AddStudentForm, ChangePasswordForm,AddTeacherForm, AddMarksForm, ModifyStudentForm, FeedbackForm, DeleteStudentForm
...
if os.environ.get('FLASK_ENV') == 'development':
    ...
    # Production on Render (take from environment variable)
...
@app.route('/teacher_password', methods=['GET', 'POST'])
...
def delete_student():
    ...
    if form.validate_on_submit():
        ...
        # Get teacher
        teacher = teachers.query.filter_by(username=teacher_username).first()

        if teacher and check_password_hash(teacher.password, teacher_password):
            ...
            if student:
                ...
                db.session.commit()
                flash(f'Student with roll no {roll_no} deleted successfully!', 'success')
                return redirect(url_for('teacher_login'))
            else:
